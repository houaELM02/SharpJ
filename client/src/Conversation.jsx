...
console.log("content "+JSON.stringify(props.userContacts[0]) );
    ...
    const conversations =usersArray.map((user, index) => (
        <div
            ...
            onClick={() => {
                props.setischatroom(false);
                var img = "http://localhost:5130/Images/Houda240728389.jpeg"
                props.setUserData({ ...props.userData, receivername: user.firstName+" "+user.lastName,receiverImg:img});
                setSelectedConversation(user.id);
            }}
        >
            <span style={{ position: "relative" }}>
                <img src={"http://localhost:5130/Images/Houda240728389.jpeg"} className="profile_pic_chat" alt="pic" />
                {user.online===true ? <span className='online'></span>:''}
